...
newtype Cloud a= Cloud {runCloud' ::TransIO a} deriving (Functor,Monoid,Applicative,Alternative, Monad, Num, MonadState EventF)
...
loggedc (Cloud mx)= Cloud $ do
    Log rep log full hash <- getData `onNothing` return (Log False [] [] 0)
        

    cont <- get  

              
    r<- logged $ mx <*** do                              -- setData closRemote
            when rep $ do
              mfdata <- gets mfData              
              Connection {localClosures=localClos} <- getData `onNothing` error "Listen: myNode not set"


              let mod x sdata=  M.insert (typeOf x) (unsafeCoerce x) sdata
                  log  = fromJust $ ext $ mfData cont :: Log
                  conn = fromJust $ ext $ mfData cont :: Connection
                  rem  = fromJust $ ext $ mfData cont :: RemoteClosures
                  was  = fromJust $ ext $ mfData cont :: RemoteStatus
                  ext sdata= r where r= unsafeCoerce $ M.lookup (typeOf $ fromJust r) sdata  
              liftIO $ modifyMVar_ localClos $ \map -> return $ M.insert hash (full,cont{mfData= mod was $ mod rem $ mod conn $ mod log mfdata} ) map
              return () !> ("HASH LOGGEDC",hash , reverse full)


            mcls <- getData 
            return () !>  ("LOGGEDC CLOSURE=",hash, isJust mcls)
            let  RemoteClosures cls first= fromMaybe (RemoteClosures M.empty True)   mcls 
                 -- necesario que las dos conexiones de un applicative esten aqui.
                 -- remoteClosures debe guardarse en un SData MVar en loggedc  el MVar 
                 newMap= M.map (\(o@(cl,n)) -> if cl > hash  then (hash, length full) else o) cls 
            
            return () !> ("LENGTH CONNECTIONS", M.size newMap)
            setState $ RemoteClosures newMap first
    do 
      Log rep log full hash <- getData `onNothing` return (Log False [] [] 0)
      when rep $ setState $ Log rep log (Exec:full) hash
    return r
...
wormhole node (Cloud comp) = loggedc $ Cloud $ Transient $ do
                ...
                Right log -> do
                  ...
                  modifyState $ \mcls ->
                      let RemoteClosures cls  first= fromMaybe (RemoteClosures M.empty  False)  mcls 
                          altr Nothing=  Just (closr,0) --length fulLog) -- lfull)
                          altr  (Just(_,n))= Just (closr,n)
                      in  Just $ RemoteClosures (M.alter altr (idConn conn )  cls)  False
